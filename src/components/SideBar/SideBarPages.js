...
export default class SideBarPages extends Component {
  ...
  setEvent() {
    ...
    this.addEvent("click", ".sidebar__pages--detail-click", async (e) => {
      ...
      switch (action) {
        case "toggle":
          const tempOpendeDetail = new Set(this.state.openedDetail);
          if (!tempOpendeDetail.has(id)) {
            tempOpendeDetail.add(id);
          } else tempOpendeDetail.delete(id);
          setItem("openedDetail", tempOpendeDetail);
          console.log("Toggle action triggered");
          // console.log(this.template());
          // console.log("현재 타겟:", this.$target);
          this.setState({ openedDetail: tempOpendeDetail });
          // this.render();
          break;
        case "remove":
